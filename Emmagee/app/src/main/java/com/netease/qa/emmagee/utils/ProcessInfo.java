...
public class ProcessInfo {
	...
	public List<Programe> getRunningProcess(Context context) {//?
		...
		for (ApplicationInfo appinfo : getPackagesInfo(context)) {//100个
			Programe programe = new Programe();
			if (((appinfo.flags & ApplicationInfo.FLAG_SYSTEM) > 0) //若为系统应用跳过
                    || ((appinfo.processName != null)
                    && (appinfo.processName.equals(PACKAGE_NAME)))) {//跳过自己
				continue;
			}
			for (RunningAppProcessInfo runningProcess : run) {//runningProcess是run中的元素
	 ...
	 * @return pid
	 */
	public Programe getProgrameByPackageName(Context context, String packageName) {//service
	 ...
	 */
	public static String getTopActivity(Context context) {//经常调用
