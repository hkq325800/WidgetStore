...
public class EmmageeService extends Service {
	...
	public void onCreate() {
		...
		fomart.setMaximumFractionDigits(2);//保留两位小数
	 ...
	 * 
	 */
	public class BatteryInfoBroadcastReceiver extends BroadcastReceiver {//没有注册
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (Intent.ACTION_BATTERY_CHANGED.equals(intent.getAction())) {
				...
				voltage = String.valueOf(intent.getIntExtra(BatteryManager.EXTRA_VOLTAGE, -1) * 1.0 / 1000);//电压
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		builder.setContentIntent(contentIntent).setSmallIcon(R.drawable.icon).setWhen(System.currentTimeMillis()).setAutoCancel(true)
				.setContentTitle("Emmagee");//构建前台通知
		...
		readSettingInfo();
		if (isFloating) {
			viFloatingWindow = LayoutInflater.from(this).inflate(R.layout.floating, null);//未显浮窗
		...
		createResultCsv();
		handler.postDelayed(task, 0);//延时启动线程
	...
	private void readSettingInfo() {
		...
		delaytime = interval * 1000;//5000
		isFloating = preferences.getBoolean(Settings.KEY_ISFLOAT, true);//true
		...
		receivers = recipients.split("\\s+");//[0]:''
		smtp = preferences.getString(Settings.KEY_SMTP, BLANK_STRING);//''
		isRoot = preferences.getBoolean(Settings.KEY_ROOT, false);//true
		isAutoStop = preferences.getBoolean(Settings.KEY_AUTO_STOP, true);//false
	}
	...
	private void createResultCsv() {
		...
		if (android.os.Environment.getExternalStorageState().equals(android.os.Environment.MEDIA_MOUNTED)) {//sd卡挂载
		...
		try {
			...
			long totalMemorySize = memoryInfo.getTotalMemory();//2015056
			String totalMemory = fomart.format((double) totalMemorySize / 1024);//1967.83
	 ...
	 */
	private void getStartTimeFromLogcat() {//经常调用
	 ...
	 * @throws IOException
	 */
	private void dataRefresh() {//经常调用
		int pidMemory = memoryInfo.getPidMemorySize(pid, getBaseContext());//14840
		long freeMemory = memoryInfo.getFreeMemorySize(getBaseContext());//678752
		String freeMemoryKb = fomart.format((double) freeMemory / 1024);//662.84
		String processMemory = fomart.format((double) pidMemory / 1024);//14.49
		String currentBatt = String.valueOf(currentInfo.getCurrentValue());//null
		// 异常数据过滤
		try {
	 ...
	 */
	private void updateViewPosition() {//经常调用
